...
bool LightLineRender::init()
{
    ...
    M_Material = Material::createWithFilename("lighting.png");
	...
	// º”‘ÿœ‡”¶µƒShaderŒƒº˛
    // º”‘ÿLightLineRender.vsh≤¢»°µ√Œƒº˛ƒ⁄»›◊÷∑˚¥Æ
    ...
    // º”‘ÿLightLineRender.fsh≤¢»°µ√Œƒº˛ƒ⁄»›◊÷∑˚¥Æ
	...
	//Ω´Ã˘Õº…Ë÷√∏¯Shader÷–µƒ±‰¡ø÷µu_texture
...
void LightLineRender::draw(Renderer* renderer, const Mat4 &transform, uint32_t flags)
{
	if (m_VertexVector)
	{
		...
		m_pMeshCommand->genMaterialID(textureID, m_ProgramState, m_RenderMesh->getVertexBuffer(),m_RenderMesh->getIndexBuffer(), m_BlendFunc);
        m_pMeshCommand->init(_globalZOrder, M_Material, m_RenderMesh->getVertexBuffer(), m_RenderMesh->getIndexBuffer(), (GLenum)m_RenderMesh->getPrimitiveType(), (GLenum)m_RenderMesh->getIndexFormat(), m_RenderMesh->getIndexCount(), Mat4(0), 0);
